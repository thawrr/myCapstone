...
namespace CapstoneBlackstone.C_SharpClasses
{
    public class Validation
    {
        ...
        public bool TestForLegalImageTypes(string fileExtension)
        {
            bool isStringContainedInList = new[] { ".png", ".gif", ".jpg", ".jpeg", ".pdf", ".pcd", ".fpx", ".tif" }.Contains(fileExtension);
        ...
        public bool validateLogin(string usernameOrPassword)
        {
            if(usernameOrPassword.Contains("'"))
            {
                return true;
            }
            else if(usernameOrPassword.Contains(":"))
            {
                return true;
            }
            else
        ...
        {
            return Regex.IsMatch(textBox.Text, "^[a-zA-Z]+$");
        }

        //checks for an empty dataSet
        ...
        public bool HasWordCount(TextBox textBox)
        {
            int wordCount = textBox.Text.Split(new char[] { ' ', '.', ':' }, StringSplitOptions.RemoveEmptyEntries).Count();
            if (wordCount > 0)
                return true;
            else
                return false;
